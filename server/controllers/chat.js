...
const addMembers = TryCatch(async(req,res,next)=>{
 ...
 const removeMembers = TryCatch(async(req,res,next)=>{
    ...
    if(!chat) return next(new ErrorHandler("Please provide chat id",404));

    if(!chat.groupChat) return next(new ErrorHandler("This is not a group chat",404));

    if(chat.creator.toString() !== req.user.toString()) 
        return next(new ErrorHandler("You are not allowed to delete member",403));
...
if(remainingMembers.length <= 3) return next(new ErrorHandler("Group must have atleast 3 members",400))
    ...
    emitEvent(
        ...
        ALERT,
        chat.members,
        `User ${user.name} has been removed from the group` 
    )
    emitEvent(req,REFETCH_CHATS,chat.members)
...
messageWithAttachments.forEach((attachment)=>attachment.forEach(({public_id})=> public_id.push(public_id)))
