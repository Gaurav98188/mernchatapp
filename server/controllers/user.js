...
const login = TryCatch(async (req,res,next)=>{
    ...
    sendToken(res,user,200,`welcome Back ${username} created`);
...
const logout = TryCatch(async (req,res)=>{
        return res.status(200).cookie("chatttu-token","",{...cookieOptions,maxAge:0}).json({
            success:true,
...
//extracting all users and friends from my chats i have chatted with
...
const sendFriendRequest = TryCatch(async (req,res,next)=>{
    ...
    await Request.create({
        sender:req.user,//who ever hitting this api
        receiver:userId,
...
const acceptFriendRequest = TryCatch(async(req,res,next)=>{
    ...
    return res.status(200).json({
        success:true,
        message:"Friend requst accepted ",
